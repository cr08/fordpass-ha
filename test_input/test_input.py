...
logdir = os.path.join(cwd, "fordpass_logs")
...
for filename in logfiles:
    ...
    with open(filename, "r") as file:
        ...
        # Call json_metrics_loop function which loops through all metrics in json and inserts any new data
        json_metrics_loop(js)
        ...
        for i in list(js["events"]):
            if "configurationResetEvent" in i:
                ...
                conn.commit()
                del updateTime
                del js["events"]["configurationResetEvent"]["conditions"]
                del ftcp_version
            elif "torqueSourceDeliveryEvent" in i:
                table = "events.torqueSourceDeliveryEvent"
                updateTime = js["events"]["torqueSourceDeliveryEvent"]["updateTime"]
                for cond in list(js["events"]["torqueSourceDeliveryEvent"]["conditions"]):
                    conditions = cond
                ftcp_version = js["events"]["torqueSourceDeliveryEvent"]["oemData"]["ftcp_version"]["stringValue"]
                sql = f"INSERT INTO {table}(updateTime, conditions, ftcp_version) VALUES(\'{updateTime}\', \'{conditions}\', \'{ftcp_version}\') ON CONFLICT (updateTime) DO NOTHING;"
                cur.execute(sql)  
                conn.commit()
                json_metrics_loop(js["events"]["torqueSourceDeliveryEvent"])
                del updateTime
                del js["events"]["torqueSourceDeliveryEvent"]["conditions"]
                del ftcp_version
                ...
                cur.execute(sql)  
                conn.commit()
                json_metrics_loop(js["events"]["remoteStartEvent"])
                del updateTime
                del js["events"]["remoteStartEvent"]["conditions"]
                del ftcp_version
